...
ed register $W vendors --add id=model-provider name="Model provider behind the Open Autonomy valve (zai)" owner=maya criticality=low data_access="source code and issue text the agents send; no customer data" assurance="terms and data-use policy reviewed; no training on inputs" last_review=2026-06-23 | grep -i error
