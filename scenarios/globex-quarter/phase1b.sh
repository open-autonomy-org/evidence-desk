...
for f in sorted(glob.glob('$W/controls/*.json')):
  ...
  ed control $W $c --owner $o | grep -i error
done
...
ed register $W risks --add id=R-3 title="An unreviewed change reaches production" description="Two engineers; a hotfix could skip review" likelihood=2 impact=4 treatment=mitigate "controls=CHG-01;CHG-03" owner=sam status=treated review_due=2027-06-23 | grep -i error
...
ed register $W vendors --update npm owner=sam criticality=low data_access="none; serves pinned dependencies" assurance="public registry; dependencies pinned by lockfile" last_review=2026-06-23 | grep -i error
