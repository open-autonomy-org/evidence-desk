...
Scenario: the payload Durable Objects namespace is deleted. Steps: restore from the latest backup into a new namespace (per the 2026-08-20 restore test), repoint the Worker binding, redeploy through the deploy-v* path. Recovery time 48 minutes against a 4-hour objective; data loss up to the last backup (objective 24 hours). Follow-up: none." ;;
    ...
    2026-09-15) clock ${1}T10:00:00Z; completeness
      ...
      for sys in repository:github cloudflare-account:cloudflare; do s=${sys%%:*}; v=${sys#*:}
        for who in sam maya; do git -C $W checkout -q main; git -C $W checkout -q $who/access-review-q3 2>/dev/null || git -C $W checkout -q -b $who/access-review-q3
          l=evidence/files/listings/$v-members-2026-09-15-for-$who.csv
          ...
          args=(); for acct in $(tail -n +2 $W/$l | cut -d, -f1); do p=${${acct%%-gx}%%@globex.test}; [ $p = deploy ] && p=; args+=(--decide $acct=keep); [ -n "$p" ] && args+=(--person $acct=$p); done
...
Attendees: Maya Chen (owner), Sam Okafor. Reviewed: the twelve internal audits of the quarter and their one finding (C6, closed by the 2026-08-20 restore test); incident replay-headers and its corrective actions; the break-glass deploy of 2026-09-10; daily check failures (HTTPS-only off 2026-08-19 to 2026-08-21); availability 99.98%; the penetration test. Decisions: personal Cloudflare tokens lose the Workers edit permission (done 2026-09-26); next restore test due by 2026-11-20." ;;
