...
deploy() { n_deploy=$((n_deploy+1)); local t=$(now); GIT_COMMITTER_DATE="$t" git -C $D/relay -c user.name=$1 -c user.email=$1@globex.test tag -a -m "deploy-v$n_deploy" deploy-v$n_deploy && gpush $D/relay deploy-v$n_deploy; git -C $D/relay show deploy-v$n_deploy:src/index.ts > $STATE/module.js; (cd $ED && timeout 60 $V attach evidence-desk-oa --root $RT -- env CF_AS=$(cat $STATE/cftok-deploy@globex.test) WORKER_MODULE=$STATE/module.js bun $S/seed.ts deploy $(tok $2) deploy-v$n_deploy $(tok $3) $ENV 2>&1 | grep -v WARN); }
...
Customer payloads are readable only through Relay's API with the customer's own key, and by the two engineers with Cloudflare account access (Maya Chen, Sam Okafor) for support and incident response. The deploy service account (deploy@globex.test) can deploy Workers but has no data access. Agents have no Cloudflare access." ;;
    ...
    2026-08-27) clock ${1}T09:30:00Z; change sam test/tenant-isolation "Regression test: replays never cross tenants"; pr_relay sam-gx test/tenant-isolation "Regression test: replays never cross tenants" maya-gx
      clock ${1}T09:50:00Z; cp $STATE/cftok-deploy@globex.test $STATE/cftok-deploy-old; seed - cftoken deploy@globex.test > $STATE/cftok-deploy@globex.test; seed maya-gx secret CLOUDFLARE_API_TOKEN
      # The previous token is revoked once the new one is in the environment secret.
      clock ${1}T09:55:00Z; (cd $ED && timeout 60 $V attach evidence-desk-oa --root $RT -- env CF_AS=$(cat $STATE/cftok-deploy-old) bun $S/seed.ts cfrevoke - 2>&1 | grep -v WARN)
      clock ${1}T10:00:00Z; record maya maya-gx sam-gx credentials/2026-08-27-deploy-token.json '{"kind":"credential","id":"deploy-token","at":"2026-08-27T09:50:00Z","by":"maya","custody_name":"CLOUDFLARE_API_TOKEN","action":"rotated","reason":"Rotated after incident replay-headers as a precaution: a new token for the deploy@globex.test service account replaced its previous one, which was revoked at 09:55"}' "Credential: deploy token rotated"
      clock ${1}T11:00:00Z
...
Tester: Northwind Security (independent). Window: 2026-08-31 to 2026-09-04. Scope: Relay API and webhook intake at relay.globex.test. Findings: 0 critical, 0 high, 1 medium (verbose error on malformed signature header; to be fixed and retested), 2 low (accepted: rate-limit headers disclose limits; missing security.txt expiry)." ;;
    2026-09-12) clock ${1}T15:00:00Z; change sam fix/signature-error "Uniform error for malformed signature headers (pen-test medium finding)"; pr_relay sam-gx fix/signature-error "Uniform error for malformed signature headers (pen-test medium finding)" maya-gx
      clock ${1}T15:30:00Z; deploy maya maya-gx sam-gx
      clock ${1}T16:00:00Z; evdoc maya maya-gx sam-gx MON-02 pentest-retest-2026-09-12.md "Penetration test retest: the medium finding is fixed" "# Retest, 2026-09-12
Northwind Security retested the medium finding (verbose error on a malformed signature header) against deploy-v7, deployed today with the fix: the error is now uniform. Closed." ;;
...
Scenario: the payload Durable Objects namespace is deleted. Steps: restore from the latest backup into a new namespace (per the 2026-08-20 restore test), point a scratch copy of the Worker at it and replay a day of test traffic; production was not touched. Recovery time 48 minutes against a 4-hour objective; data loss up to the last backup (objective 24 hours). Follow-up: none." ;;
    2026-09-10) clock ${1}T18:20:00Z; git -C $D/relay checkout -q main && timeout 30 git -C $D/relay pull -q --no-rebase origin main 2>&1 | grep -v "no common"
      # Sam's laptop: main as it stood, plus the line he later opened as the replay-limit pull request.
      { git -C $D/relay show main:src/index.ts; echo "// Raise the replay limit (the change deployed from a laptop on 2026-09-10)"; } > $STATE/laptop.js
      (cd $ED && timeout 60 $V attach evidence-desk-oa --root $RT -- env CF_AS=$(cat $STATE/cftok-sam@globex.test) WORKER_MODULE=$STATE/laptop.js bun $S/seed.ts wrangler - "raise the replay limit (from Sam's laptop)" 2>&1 | grep -v WARN) ;;
