...
1. Copy `container/build-world.json` beside the project and replace the build service's `cwd` with
   ...
   Hermes base and then `evidence-desk-agent:local`. Check `volter-world doctor evidence-desk-image
   --root <world-state-root>`, then `volter-world down evidence-desk-image --root <world-state-root>`
...
4. Install the reviewed kit's `.open-autonomy` directory in host-owned storage and run:
   ...
   ```bash
   bun .open-autonomy/start.ts --container oa-evidence-desk --secrets /absolute/protected/project-credentials
   ```
