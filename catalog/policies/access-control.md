...
{{organization}} limits access control based on job requirements, following the principle of least privilege.

### Scope

This policy applies to {{organization}}’s internal systems, including its production network, production servers, and SaaS applications.
...
{{organization}} must implement monitoring on its systems where possible, to record logon attempts and failures, successful logons and date and time of logon and logoff. Activities performed as administrator are logged where it is feasible to do so.
...
{{organization}} systems must have sufficient granularity to allow appropriate authorized access and all access requests require approval from the Security team. There is a delicate balance between protecting the data and permitting access to those who need to use the data for authorized purposes. {{organization}} recognizes that balance.

#### End user devices

Employees, contractors, and vendors are responsible for safe handling and storage of {{organization}}-provided end user devices. If a device is lost or stolen, the loss must be immediately reported as an incident.
...
{{organization}}’s Security team is responsible for administering access to systems and reviewing and updating the Access Control process on an annual basis.
