...
To avoid potential security incidents, {{organization}} requires employees to follow password requirements.

### Scope

This policy applies to passwords for any Software as a Service (SaaS), cloud environments, and owned or leased applications or servers accessed by {{organization}} employees, contractors, or vendors. _It does not apply to the passwords customers of {{organization}} use to access the {{organization}} service._
...
Access to servers, for both production as well as development and testing infrastructure, must be with a password and MFA or with per-user public keys (e.g., SSH keys). Only {{organization}}-based network authentication is permitted for services not exposed to the Internet.
...
{{organization}}’s Security team is responsible for reviewing and updating the Password requirements on an annual basis.
