...
function lintDescription(ws: Workspace, e: Engagement, text: string, assertionText = ''): { rule: string; status: 'pass' | 'contradiction' | 'not applicable'; detail: string }[] {
  ...
  const reviewClaim = [...prose.matchAll(/\b(?:(?:every|each|all)\s+(?:code\s+)?(?:changes?|pull requests?|PRs?|merges?|code)\b|(?:changes?|pull requests?|PRs?|merges?|code)\b(?=(?:[^.\n]|\.(?=\S))*\b(?:only|always)\b))((?:[^.\n]|\.(?=\S))*?)\breview/gi)]
    .find((m) => !/\b(release|production|deploy)/i.test(m[1]));
