...
try {
  ...
  const edits = ["item-create", "item-update", "engagement", "control-create", "control-update", "due-date"];
  if (!folder || !path || !["--readiness-create", "--readiness-open"].includes(mode) ||
      !["init", "validate", "summary", "serve", ...edits].includes(operation)) {
    throw new Error("Usage: bun run src/readiness.ts <init|validate|summary|serve|engagement|control-create|control-update|due-date|item-create|item-update> <folder> <--readiness-create|--readiness-open> <exact-relative-path> [existing-id] [--stdin] [--new-workspace (serve only)]");
  }
  const needsId = ["item-update", "control-update", "due-date"].includes(operation);
  ...
  } else if (operation === "summary") console.log(JSON.stringify(readinessReport(loaded.manifest, loaded.data), null, 2));
  else console.log(`Valid format-1 workspace and explicitly selected readiness v1: ${selected.relative}. Selection is not proof of provenance or prior ownership.`);
} catch (error) {
