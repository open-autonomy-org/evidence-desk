...
Evidence Desk's provisional identity uses a portable document in an open folder: pale paper and mint
on a dark ink background. `icon.svg` is the editable source and `icon.png` is its 1024-pixel integration
avatar. The mark represents the owner's evidence folder, not an audit certification. The working name
was accepted in owner-led setup on 2026-09-08; the blurb follows `CONSTITUTION.md`.
